...
def test_image_attachment(tmp_path, monkeypatch):
    ...
    img.write_bytes(base64.b64decode(
        'iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mP8/x8AAwMCAO+kBlcAAAAASUVORK5CYII='))
...
def test_read_pdf_limits(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(lair.config, 'active', {**lair.config.active, 'misc.text_attachment_max_size': 5, 'misc.text_attachment_truncate': True})
...
def test_pdf_and_text_files(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(lair.config, 'active', {**lair.config.active, 'misc.text_attachment_max_size': 3, 'misc.text_attachment_truncate': True})
