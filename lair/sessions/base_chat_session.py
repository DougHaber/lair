...
class BaseChatSession(abc.ABC):
    ...
    @abc.abstractmethod
    def __init__(self, *, history: Optional[ChatHistory] = None, tool_set: Optional[ToolSet] = None) -> None:
        """Initialize the chat session.

        Parameters
        ----------
        history : ChatHistory, optional
            History instance to use. If ``None``, a new ``ChatHistory`` is created.
        tool_set : ToolSet, optional
            Tool set to use. If ``None``, a new ``ToolSet`` is created.
        ...
        self.session_id: Optional[int] = None  # Id for session management, provided by SessionManager()
        self.session_alias: Optional[str] = None  # Alias string for session management purposes
        self.session_title: Optional[str] = None  # Short title for the session
    ...
    def chat(self, message: Optional[Union[str, list[dict[str, Any]]]] = None) -> str:
        """Add a message, invoke the model, and return the response.

        Parameters
        ----------
        message : str | list[dict[str, Any]] | None
            The message to process. May be a raw string, a list of message
            dictionaries, or ``None`` to send no additional message.

        Returns
        -------
        str
            The response generated by the chat completion request.
    ...
    def _add_message_to_history(self, message: Union[str, list[dict[str, Any]]]) -> None:
        if isinstance(message, str):
            self.history.add_message("user", message)
        elif isinstance(message, list):
            self.history.add_messages(message)

    def _invoke_chat(self) -> tuple[str, Optional[list[dict[str, Any]]]]:
        if lair.config.get("tools.enabled"):
            return self.invoke_with_tools()
        return self.invoke(), None

    def _record_response(self, answer: str, tool_messages: Optional[list[dict[str, Any]]]) -> None:
        if tool_messages:
            self.history.add_tool_messages(tool_messages)
        self.history.add_message("assistant", answer)
        self.history.commit()

    def auto_generate_title(self) -> Optional[str]:
        """Generate a short title for the current session.

        Returns
        -------
        str | None
            The generated title, or ``None`` if generation failed or is disabled.
    ...
    def new_session(self, preserve_alias: bool = False, preserve_id: bool = False) -> None:
        ...
        Parameters
        ----------
        preserve_alias : bool, optional
            If ``True``, the ``session_alias`` is kept when clearing the session.
        preserve_id : bool, optional
            If ``True``, the ``session_id`` is kept when clearing the session.
