...
class ChatInterfaceCommands():
    ...
    def command_extract(self, command, arguments, arguments_str):
        if len(arguments) > 2:
            ...
            if self.conversation_manager.last_response:
                response = self._get_embedded_response(self.conversation_manager.last_response, position)
                if response:
    ...
    def command_history(self, command, arguments, arguments_str):
        if len(arguments) != 0:
            ...
            messages = self.conversation_manager.history.get_messages()
    ...
    def command_history_slice(self, command, arguments, arguments_str):
        if len(arguments) != 1:
            ...
            self.conversation_manager.history.set_history(messages)
    ...
    def command_last_prompt(self, command, arguments, arguments_str):
        if len(arguments) > 1:
            ...
            last_prompt = self.conversation_manager.last_prompt
            if last_prompt:
    ...
    def command_last_response(self, command, arguments, arguments_str):
        if len(arguments) > 1:
            ...
            last_response = self.conversation_manager.last_response
            if last_response:
                filename = arguments[0] if len(arguments) == 1 else None
                if filename is not None:
    ...
    def command_list_models(self, command, arguments, arguments_str):
        if len(arguments) != 0:
            ...
            models = sorted(self.conversation_manager.list_models(), key=lambda m: m['id'])
    ...
    def command_mode(self, command, arguments, arguments_str):
        if len(arguments) == 0:  # Show modes
            ...
            old_session = self.conversation_manager
    ...
    def command_model(self, command, arguments, arguments_str):
        if len(arguments) > 1:
            ...
            self.reporting.table_system([
                ['Name', self.conversation_manager.fixed_model_name or active_config.get('model.name')],
